...
public class AnswerWorker extends Task<Map<String, Integer>> {
    ...
    protected Map<String, Integer> call() throws Exception {
        map.clear();
        isSearched = false;
//        if(new File(ApplicationRunner.DIR_TO_SAVE).list().length != 0){
//            for(File f : new File(ApplicationRunner.DIR_TO_SAVE).listFiles()){
//                f.delete();
//            }
//        }
        searchOnLocalDrive();
...
//        if(Files.exists(Paths.get(ApplicationRunner.DIR_TO_SAVE))){
//            URL url = new URL(urlStr);
//            ReadableByteChannel rbc = Channels.newChannel(url.openStream());
//            FileOutputStream fos = new FileOutputStream(ApplicationRunner.DIR_TO_SAVE + file);
